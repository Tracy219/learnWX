...
class HelloFrame(wx.Frame):
	...
	def OnSubmitComplex(self, e):
		...
		if len(name) > 0:
			...
			wx.MessageBox("Hey, you didn't enter a name! I'll call you No name.", "Info", wx.OK | wx.CANCEL)
			self.nameBox.SetValue("No name")
...
#0. Remember to read through all the code and comments to at least get the basic idea before starting the exercises. Done!

#1. What is the purpose of line 12? Write you answer below. Writing helps clearly identify answers.
# Set up the information foe the frame, that's we only need to enter parent in the main program, 
# but not the ID number and the title.

#2. The submit button is currently bound to a simple even handler called OnSubmitSimple. Done!
...
#3. The last argument on line 58 is wx.OK That tells the program to include an "OK" button. Done!
...
#4. This one is not related to text boxes or message boxes, but it is interesting. Done!
...
#5. If the user doesn't enter a name, we can give them a default name. Done!
#   Change the info message to "Hey, you didn't enter a name! I'll call you Dave."

#6. Good job on #5. It was a little easy. Now we need to actually call the user Dave. Done!
